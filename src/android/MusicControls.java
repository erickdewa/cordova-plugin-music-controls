...
public class MusicControls extends CordovaPlugin {
    ...
    private void registerBroadcaster(MusicControlsBroadcastReceiver mMessageReceiver) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            context.registerReceiver((BroadcastReceiver) mMessageReceiver, new IntentFilter("music-controls-seek-to"), Context.RECEIVER_EXPORTED);

            // Listen for headset plug/unplug
            context.registerReceiver((BroadcastReceiver) mMessageReceiver, new IntentFilter(Intent.ACTION_HEADSET_PLUG), Context.RECEIVER_EXPORTED);

            // Listen for bluetooth connection state changes
            context.registerReceiver((BroadcastReceiver) mMessageReceiver, new IntentFilter(android.bluetooth.BluetoothHeadset.ACTION_CONNECTION_STATE_CHANGED), Context.RECEIVER_EXPORTED);
        }
    }
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("create")) {
            ...
            setElapsed = Integer.parseInt(infos.elapsed) * 1000;
            ...
            this.cordova.getThreadPool().execute(new Runnable() {
                public void run() {
                    ...
                    metadataBuilder.putLong(MediaMetadataCompat.METADATA_KEY_DURATION, (int) (doubleValDuration * 1000));
    ...
    private void setMediaPlaybackState(int state) {
        ...
        if (state == PlaybackStateCompat.STATE_PLAYING) {
            ...
            playbackstateBuilder.setActions(PlaybackStateCompat.ACTION_PLAY_PAUSE | PlaybackStateCompat.ACTION_PAUSE | PlaybackStateCompat.ACTION_SKIP_TO_NEXT | PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS |
                ...
                PlaybackStateCompat.ACTION_PLAY_FROM_SEARCH | PlaybackStateCompat.ACTION_SEEK_TO);
            playbackstateBuilder.setState(state, setElapsed, 1.0f, SystemClock.elapsedRealtime());
        } else {
            setElapsed += SystemClock.elapsedRealtime() - startElapsedAt;
            playbackstateBuilder.setActions(PlaybackStateCompat.ACTION_PLAY_PAUSE | PlaybackStateCompat.ACTION_PLAY | PlaybackStateCompat.ACTION_SKIP_TO_NEXT | PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS |
                PlaybackStateCompat.ACTION_PLAY_FROM_MEDIA_ID |
                PlaybackStateCompat.ACTION_PLAY_FROM_SEARCH | PlaybackStateCompat.ACTION_SEEK_TO);
            playbackstateBuilder.setState(state, setElapsed, 0, SystemClock.elapsedRealtime());
        }
